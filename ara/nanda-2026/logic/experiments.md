...
**Procedure:** Compute instrument minus helper final sealed reward per complete block. Apply the manifest's missing-reward rules. Parse Haiku and Sonnet from their machine-generated endpoint tables, Opus block 1 from the two digest-bound capsules, and Opus blocks 2 and 3 from the committed pre-probe summary. Preserve source precision and evidence class per row; do not reconstruct missing Opus secondary endpoints.
...
**Evidence:** `evidence/results/paper-results.json` and `.md`, generated by `src/execution/build_paper_results.py` from pinned committed sources. Opus block 1 carries reward-receipt digests; block 2--3 receipts and secondary endpoint tables remain pending; see `evidence/results/reconciliation-status.md`.

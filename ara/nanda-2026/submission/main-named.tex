...
\author{
\IEEEauthorblockN{Oliver Muellerklein\IEEEauthorrefmark{1}}
\IEEEauthorblockA{University of California, Berkeley\\omuellerklein@berkeley.edu}
\and
\IEEEauthorblockN{Chenming Tang\IEEEauthorrefmark{1}}
\IEEEauthorblockA{Proofpress\\tangchenming0830@gmail.com}
...
Our running example is an AI coding agent improving a 2048 program: it edits, tests, and keeps or reverts versions. RSI-Exam provides visible evaluations and grades submitted programs on unseen data~\cite{rsiexam}. Self-improvement here concerns the task program, not model weights. Each kept version may become the parent of later edits.
...
The producer hands over saved code versions, test results, a decision log, submitted code, and a machine-readable \emph{capsule}. Decisions are recorded during the run; the capsule is built afterward. The recipient checks these files without the producing runtime.
...
In the synthetic package (Table~\ref{tab:example}), both candidates derive from v1. Candidate v2 is reverted; v3 requires confirmation before being kept and submitted.
...
OpenAI Codex and Anthropic Claude Code assisted extensively with drafting and revising the abstract and Sections I--VI. Codex also assisted with code for the Section IV study and demonstration, artifact tooling, test execution, and typesetting checks; Claude Code assisted with manuscript revisions and the source for Fig.~\ref{fig:boundaries}. The authors take responsibility for the final content.
...
\bibitem{trace} O. Muellerklein, \emph{TRACE: A decision-provenance record for human and AI work}, v0.5.1, Apache-2.0, 2026. [Online]. Available: \url{https://github.com/Thru-Echoes/TRACE}, doi: 10.5281/zenodo.21711455.
\bibitem{proofpress} C.-M. Tang, \emph{Proofpress: An evidence ledger with separate claim review and governed reuse}, Apache-2.0, 2026. [Online]. Available: \url{https://github.com/chenmingtang830/proofpress}.
